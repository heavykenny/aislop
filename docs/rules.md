...
only recognised as code in the logging call's own arguments, so neither a
message that mentions `exc_info=False` nor a helper called with it inside
those arguments cancels the exemption.
