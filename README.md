...
[![npm version](https://img.shields.io/npm/v/slop.svg)](https://www.npmjs.com/package/slop)
[![CI](https://github.com/heavykenny/slop/actions/workflows/ci.yml/badge.svg)](https://github.com/heavykenny/slop/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)

AI coding assistants are fast, but they leave behind a trail of sloppy code: swallowed exceptions, trivial comments that restate the obvious, unused imports, `as any` casts, empty catch blocks, leftover `console.log` calls, and copy-paste duplication. **slop** catches all of it in one command.

`slop` is a unified code-quality CLI for multi-language repositories. It runs formatting, linting, code-quality, AI-pattern, architecture, and security checks behind a single command and summarizes everything as one score out of 100.
...
Code generated by AI assistants often looks correct at first glance but is full of quality issues that pass review because they are spread across dozens of files. No single existing linter catches all of them. `slop` was built to be the one tool that does:
...
- **Zero config to start** -- run `npx slop scan` and get results immediately
...
`slop` ships with Node-based tooling (oxlint, biome, knip) as package dependencies. On install it also downloads bundled binaries for **ruff** and **golangci-lint**. To skip those downloads:

```bash
SLOP_SKIP_TOOL_DOWNLOAD=1 npm install
```

Some checks depend on tools already present on your machine: `gofmt`, `govulncheck`, `cargo`, `rubocop`, `phpcs`, `php-cs-fixer`. Run `slop doctor` to see what is available.
...
npx slop scan --changes

# Scan only staged files
npx slop scan --staged
...
|---|---|
| `slop` | Interactive TTY menu (falls back to `scan` in non-TTY) |
| `slop scan [dir]` | Run all enabled engines and print a scored report |
| `slop fix [dir]` | Apply safe auto-fixes (formatting + lint) |
| `slop ci [dir]` | Output JSON for CI pipelines |
| `slop init [dir]` | Create `.slop/config.yml` and `.slop/rules.yml` |
| `slop doctor [dir]` | Report which tools are installed and available |
| `slop rules [dir]` | List all built-in and configured rules |
...
`slop` groups checks into six engines. Each engine runs in parallel for speed.
...
The rules that make slop unique. These catch the patterns AI assistants leave behind.
...
Custom import and path rules defined in `.slop/rules.yml`.
...
Run `slop init` to generate `.slop/config.yml`:
...
Set `ci.failBelow` in `.slop/config.yml`:
...
`slop ci` exits with code 1 when the score is below the threshold.
